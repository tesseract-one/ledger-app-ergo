...
static void test_buffer_can_read(void **state) {
    ...
    uint8_t temp[20] = {0};
    buffer_t buf;
    buffer_init(&buf, temp, sizeof(temp), sizeof(temp));
...
static void test_buffer_read(void **state) {
    ...
    buffer_t buf_varint;
    buffer_init(&buf_varint, temp_varint, sizeof(temp_varint), sizeof(temp_varint));
    uint64_t varint = 0;
    // TODO: buffer read varint
    // assert_true(buffer_read_u64(&buf_varint, &varint, LE));
    ...
    // assert_false(buffer_read_u64(&buf_varint, &varint, LE));
}
