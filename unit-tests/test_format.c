...
static void test_format_i64(void **state) {
    ...
    int64_t value = 0;
    assert_int_equal(format_i64(temp, sizeof(temp), value), 1);
    assert_string_equal(temp, "0");

    value = (int64_t) 9223372036854775807ull;  // MAX_INT64
    memset(temp, 0, sizeof(temp));
    assert_int_equal(format_i64(temp, sizeof(temp), value), 19);
    assert_string_equal(temp, "9223372036854775807");

    // buffer too small
    assert_int_equal(format_i64(temp, sizeof(temp) - 5, value), -1);

    value = (int64_t) -9223372036854775808ull;  // MIN_INT64
    memset(temp, 0, sizeof(temp));
    assert_int_equal(format_i64(temp, sizeof(temp), value), 20);
...
static void test_format_u64(void **state) {
    ...
    uint64_t value = 0;
    assert_int_equal(format_u64(temp, sizeof(temp), value), 1);
    assert_string_equal(temp, "0");

    value = (uint64_t) 18446744073709551615ull;  // MAX_UNT64
    memset(temp, 0, sizeof(temp));
    assert_int_equal(format_u64(temp, sizeof(temp), value), 20);
    assert_string_equal(temp, "18446744073709551615");

    // buffer too small
    assert_int_equal(format_u64(temp, sizeof(temp) - 5, value), -1);
}
...
static void test_format_fpu64(void **state) {
    ...
    // buffer too small
    assert_int_equal(format_fpu64(temp, sizeof(temp) - 16, amount, 8), -1);

    char temp2[50] = {0};

    amount = 1000000000000000000ull;  // wei
    assert_int_equal(format_fpu64(temp2, sizeof(temp2), amount, 18), 20);
    assert_string_equal(temp2, "1.000000000000000000");  // ETH

    // buffer too small
    assert_int_equal(format_fpu64(temp2, sizeof(temp2) - 20, amount, 18), -1);
}
